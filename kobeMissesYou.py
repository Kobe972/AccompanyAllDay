...
class MyWidget(QWidget):
    ...
    def on_status_triggered(self, action):
        ...
        new_status=json.loads(requests.post('http://47.108.160.172:7002/setStatus',data=data,proxies={}).text)
        if self.role==1:
    ...
    def upload_file(self, file_path):
        with open(file_path, "rb") as f:
            ...
            data = {"id":self.role}
            try:
                if(requests.post("http://47.108.160.172:7002/uploadPhoto", data=data, files=files, proxies={}).text=="Success"):
    ...
    def on_delete_triggered(self):
        data={"id":self.role}
        try:
            if(requests.post("http://47.108.160.172:7002/deletePhoto", data=data, proxies={}).text=="Success"):
    ...
    def on_view_photo_triggered(self, action):
        ...
        response = requests.get('http://47.108.160.172:7002/getPhoto', params={"id": id}, proxies={})
...
while True:
    try:
        status1=json.loads(requests.get('http://47.108.160.172:7002/getStatus1',proxies={}).text)
        status2=json.loads(requests.get('http://47.108.160.172:7002/getStatus2',proxies={}).text)
        break
    except:pass
